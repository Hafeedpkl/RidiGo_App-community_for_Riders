...
class RidesRegistered extends StatelessWidget {
  const RidesRegistered({super.key});
        ...
        child: Consumer<PostProvider>(builder: (context, value, _) {
          if (value.isLoading == true) {
            ...
            return value.registeredRides != null
                ? ListView.builder(
                    itemCount: value.registeredRides.length,
                    itemBuilder: (context, index) {
                      List regMembers = value.registeredRides[index].regMembers;
                      bool isRegistered =
                          value.checkRegistered(regMembers: regMembers);
                      value.openGroup(
                          groupId: value.registeredRides[index].group);
                      final daysLeft = getDaysleft(
                          value.registeredRides[index].expirationDate);
                      final data = value.registeredRides[index];
                                ...
                                Padding(
                                  ...
                                  child: Row(
                                    children: [
                                      ...
                                      checkWishList(
                                          postId:
                                              value.registeredRides[index].id),
                                      isRegistered == false
                                          ? ElevatedButton(
